      ...
      <div className="glass-card overflow-hidden rounded-2xl shadow-premium">
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {scans.map((scan) => (
                <tr key={scan.id} className="border-b border-white/5 hover:bg-white/3">
                  <td className="max-w-xs truncate px-4 py-3 text-white">{scan.target}</td>
                  <td className="px-4 py-3 text-xs text-scanonix-muted">{scan.userEmail ?? scan.userId.slice(0, 8)}</td>
                  <td className="px-4 py-3 text-neutral-200">{scan.riskScore}</td>
                  <td className="px-4 py-3 capitalize text-neutral-200">{scan.status}</td>
