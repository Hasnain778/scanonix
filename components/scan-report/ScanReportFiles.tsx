...
const FILE_ROWS = [
  { key: "scanned" as const, label: "Files scanned", icon: "📁", tone: "text-foreground" },
...
export function ScanReportFiles({ files }: ScanReportFilesProps) {
  return (
    <section aria-labelledby="files-heading">
      <h3 id="files-heading" className="mb-5 text-lg font-semibold text-foreground">
        Files analysed
      </h3>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {FILE_ROWS.map((row) => (
          ...
          >
            <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-surface-muted/80 text-lg" aria-hidden="true">
              {row.icon}
            </span>
            <div>
