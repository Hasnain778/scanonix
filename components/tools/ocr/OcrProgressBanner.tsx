import type { OcrProgressPhase } from "@/lib/tools/ocr/languages";
import { getOcrProgressMessage } from "@/lib/tools/ocr/languages";
import type { ToolStatus } from "@/lib/tools/types";

interface OcrProgressBannerProps {
  status: ToolStatus;
...
export function OcrProgressBanner({
  status,
  phase,
  message,
  progress,
}: OcrProgressBannerProps) {
  if (status === "idle") {
    return null;
  }
  ...
  const loadingMessage =
    message ??
    (phase
      ? getOcrProgressMessage(phase, progress)
      : progress
  ...
  const defaultMessages = {
    ...
    success: message ?? "Complete — text extracted successfully!",
