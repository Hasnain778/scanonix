...
const ACCEPT_IMAGES = ".png,image/png";
...
function isSupportedPng(file: File): boolean {
...
function transparencyLabel(hasAlpha: boolean | undefined): string {
  if (hasAlpha === true) return "Preserved";
  if (hasAlpha === false) return "None";
  return "Unknown";
}
...
export function PngToPsdTool() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    const attempt = createProcessAttempt("png-to-psd");
    if (!attempt?.markStarted()) return;

    setStatus("loading");
    setMessage("Converting PNG into a PSD file…");
    setResultBlob(null);
    setStats(undefined);
    ...
    const result = await submitImageToolForm("/api/tools/png-to-psd", formData);
    if (!result.ok) {
  ...
  const handleDownload = useCallback(() => {
    ...
    downloadBlob(
      ...
      buildToolDownloadMeta("png-to-psd", 1),
    );
    setStatus("success");
        ...
        empty={
          <>
            <FileDropZone
              ...
              multiple={false}
              label="Drop a PNG to convert"
              hint={`PNG — up to ${MAX_MB}MB · Single-frame only · Output is a single raster layer PSD`}
              disabled={isBusy}
              validateFile={isSupportedPng}
              icon={<ConvertDropIcon />}
              onInvalidFiles={() => {
                setMessage(
                  `Please choose a PNG image up to ${MAX_MB}MB. Animated PNG (APNG) is not supported.`,
                );
                setStatus("error");
              }}
            ...
            <div className="pr-32 sm:pr-40 lg:pr-0">
              <p className="mb-3 text-sm text-scanonix-muted">
                Convert PNG images into PSD files with your image on a single raster
                layer. PNG transparency is preserved when present. Animated PNG is
                not supported.
              </p>
                ...
                {previewUrl ? (
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-scanonix-muted">
                      Convert PNG images into PSD files with your image on a single
                      raster layer. PNG transparency is preserved when present.
                    </p>
                  </div>
                ) : null}
              </div>
        ...
        controlPanel={
          file ? (
            hasResult && resultBlob && stats ? (
                ...
                */}
                <div className="min-h-0 flex-1 space-y-2.5 overflow-y-auto overscroll-contain py-3 pl-3.5 pr-40 sm:py-3 sm:pl-4 sm:pr-44 lg:pl-4 lg:pr-4">
                  ...
                  <dl className="divide-y divide-border/70 overflow-hidden rounded-lg border border-border bg-surface-muted/60 text-sm">
                    ...
                    <div className="flex justify-between gap-3 px-3 py-1.5">
                      <dt className="text-scanonix-muted">Transparency</dt>
                      <dd className="min-w-0 break-words text-right font-semibold text-foreground">
                        {transparencyLabel(stats.hasAlpha)}
                      </dd>
                    </div>
                    {resultFileName ? (
                  ...
                  <p className="pr-2 text-[11px] leading-snug text-scanonix-muted lg:pr-0">
                    PNG files are flattened raster images. Original Photoshop text,
                    smart objects, vector objects, or editable design layers cannot be
                    reconstructed.
                  </p>
                </div>
              ...
              <ToolControlPanel
                aria-label="PNG to PSD controls"
                footer={
                  <div className="flex flex-col gap-2 pr-32 sm:pr-40 lg:pr-0">
                    <p className="text-[11px] leading-snug text-scanonix-muted">
                      Output is a standard PSD with one raster layer. Transparency is
                      preserved when present. Animated PNG is not supported.
                    </p>
