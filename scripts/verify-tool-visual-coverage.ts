...
function run() {
  ...
  assert(
    "Canonical tool count is 37",
    CANONICAL_TOOL_IDS.length === 37,
    `got ${CANONICAL_TOOL_IDS.length}`,
  );

  assert(
    "TOOL_VISUALS covers all 37 canonical tools",
    missing.length === 0,
  ...
  assert(
    "TOOL_VISUALS has exactly 37 entries",
    TOOL_VISUAL_SLUGS.length === 37,
